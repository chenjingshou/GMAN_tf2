...
if __name__ == "__main__":
    ...
    parser.add_argument("--L", type=int, default=5, help="number of STAtt Blocks") # 注意确认此处L是否与训练时一致
    ...
    parser.add_argument("--batch_size", type=int, default=32) # 评估时可以稍微调大batch_size
    parser.add_argument("--frechet_dim", type=int, default=16, help="Fréchet embedding dimension")
    
    # 这里的参数其实评估时用不到，但也留着防报错
    parser.add_argument("--attn_dropout", type=float, default=0.0)
    parser.add_argument("--block_dropout", type=float, default=0.0)
    parser.add_argument("--use_ln", default=True, type=bool)
    parser.add_argument("--aug_enable", default=False, type=bool) # 评估关闭增强
    parser.add_argument("--aug_noise_std", type=float, default=0)
    parser.add_argument("--aug_rw_std", type=float, default=0)

    # 文件路径 - 已修改为你指定的绝对路径
    parser.add_argument("--traffic_file", default="data/METR.h5")
    parser.add_argument("--SE_file", default="data/SE(METR).txt")
    parser.add_argument("--model_file", default="/mnt/yunpan/GMAN/gman_tf2/models/DishTS_GMAN.weights.h5") 
    parser.add_argument("--log_file", default="./log/DishTS_GMAN_Eval_Result.log")
    ...
    log_string(log_f, f"**** Evaluation Mode ****")
    log_string(log_f, str(args))

    # 1. 数据加载
    log_string(log_f, "Loading data...")
    (
        trainX, trainTE, trainFrechet, trainY,
        valX, valTE, valFrechet, valY,
        testX, testTE, testFrechet, testY,
        SE, mean, std,
    ) = load_data(args)
    
    log_string(log_f, f"testX: {testX.shape}\ttestY: {testY.shape}")
    ...
    # 2. 模型实例化
    log_string(log_f, "Instantiating model...")
    model = DishTS_GMAN_Plus(args, SE, mean, std)

    # 3. 关键步骤：先跑一个Dummy Batch来构建图，否则load_weights可能会因为shape不明而报错
    log_string(log_f, "Building model graph...")
    dummy_x, dummy_y = next(iter(test_ds))
    _ = model(dummy_x, training=False) 
    
    # 4. 加载权重
    log_string(log_f, f"Loading weights from {args.model_file}...")
    try:
        model.load_weights(args.model_file)
        log_string(log_f, "Weights loaded successfully!")
    except Exception as e:
        log_string(log_f, f"Error loading weights: {e}")
        log_string(log_f, "Please check if the model architecture parameters (L, K, d) match the training settings.")
        exit()
    ...
    end_test = time.time()
    log_string(log_f, f"Testing time: {end_test - start_test:.1f}s")

    # 6. 计算整体指标
    test_mae, test_rmse, test_mape = metric(test_pred, testY)
    
    log_string(log_f, "                MAE\t\tRMSE\t\tMAPE")
    log_string(log_f, "test            %.2f\t\t%.2f\t\t%.2f%%" % 
               (test_mae, test_rmse, test_mape * 100))

    # 7. 计算每一步的指标 (Step-by-step Performance)
    log_string(log_f, 'performance in each prediction step')
    MAE_list, RMSE_list, MAPE_list = [], [], []
    
    for q in range(args.Q):
        # 取出第 q 步的预测值和真实值
        # shape通常是 (Batch, Time, Nodes) -> 取切片变为 (Batch, Nodes)
        pred_step = test_pred[:, q, ...]
        true_step = testY[:, q, ...]
        
        mae_s, rmse_s, mape_s = metric(pred_step, true_step)
        
        MAE_list.append(mae_s)
        RMSE_list.append(rmse_s)
        MAPE_list.append(mape_s)
        
        log_string(log_f, 'step: %02d        %.2f\t\t%.2f\t\t%.2f%%' % 
                   (q + 1, mae_s, rmse_s, mape_s * 100))

    # 8. 计算平均值 (Double Check)
    average_mae = np.mean(MAE_list)
    average_rmse = np.mean(RMSE_list)
    average_mape = np.mean(MAPE_list)
    
    log_string(
        log_f, 'average:         %.2f\t\t%.2f\t\t%.2f%%' %
        (average_mae, average_rmse, average_mape * 100))

    log_f.close()
    print("Evaluation finished! Check the log file.")
